...
// Normalize text: remove html tags, excess spaces, strip newlines
...
/**
 * Baseline Seed Data - Ensures the bot has high-quality, non-hallucinating 
 * official UT info immediately upon startup, even if live scraping is rate-limited.
 */
const BASELINE_SEED = {
  faq: [
    {
    ...
    },
    {
      ...
      answer: "Jadwal Tutorial Online (Tuton) biasanya dimulai sekitar 1-2 bulan setelah registrasi ditutup. Tanggal pasti dapat dilihat pada Kalender Akademik UT di website resmi. Mahasiswa wajib melakukan aktivasi akun Tuton terlebih dahulu di elearning.ut.ac.id dan melakukan pengisian form Kesediaan Mengikuti Tuton.",
...
export async function seedBaselineData() {
  ...
  // Seed Whitelisted Groups (Empty by default)
...
/**
 * Extract internal links pointing to the same website domain.
 */
  ...
  try {
    ...
    $('a').each((_, el) => {
      ...
      try {
        ...
        // Only crawl exact same host
        if (parsedUrl.hostname === parsedBase.hostname) {
          // Exclude media, files, logins, logouts, admin panel, and hashtag anchors
          if (
...
/**
 * Scrapes a single URL page content and extracts articles/FAQs
 */
async function scrapeSingleUrl(url, category) {
  ...
  try {
    ...
    // Remove unwanted script, style, navigation, footer and sidebar elements
    $('script, style, iframe, nav, footer, header, .sidebar, #footer, #header, .menu, .ads, noscript').remove();

    const crawledItems = [];
    
    // 1. Try to find articles or specific content containers
    ...
    // 2. Try to find FAQ-style accordions or lists
    ...
    // 3. Fallback: Parse headings and page content
...
/**
 * Scrapes a target URL and follows its internal links up to a limit (Depth = 1)
 */
...
/**
 * Runs the crawler across all target URLs and updates the local database collections.
 */
export async function runCrawler() {
  logger.info("Starting UT Academic Crawler Job...");
  
  // Make sure baseline seed is present first
  ...
  if (totalCrawled > 0) {
    ...
    newFaq.forEach(item => {
      if (!combinedFaq.some(x => x.question.toLowerCase() === item.question.toLowerCase())) {
    ...
    logger.warn("Crawler job finished, but zero new content was extracted (using seeds and existing cache).");
  }
  
  return totalCrawled;
}

// Run crawler manually if script executed directly
