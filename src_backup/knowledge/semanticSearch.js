...
class TFIDFSearchIndex {
  constructor() {
    this.documents = []; // array of { id, text, docRef }
    this.idf = {};       // term -> idf
    this.tfIdfVectors = []; // array of { docId, vector: { term -> weight } }
    this.isBuilt = false;
  }
  ...
  async search(query, limit = 3) {
    ...
    this.tfIdfVectors.forEach(({ docId, vector }) => {
      ...
      // If critical academic keywords appear exactly in the document title or content, boost similarity
      ...
      if (doc) {
        ...
        const utKeywords = ['uobm', 'uolp', 'uaop', 'utm', 'the', 'tuton', 'tuweb', 'myut', 'registrasi', 'lip'];
